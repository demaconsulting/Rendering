...
internal sealed class LayeredCorridorRouter : ILayoutStage
{
    ...
    private static void CreateDependency(Segment s1, Segment s2)
    {
        ...
        else if (c2 < c1)
        {
            ...
            && Math.Abs(s1.TargetY - s2.TargetY) > StraightTolerance
            && Math.Abs(s1.SourceY - s2.SourceY) > StraightTolerance)
        {
            // Equal non-zero crossings: unavoidable conflict — pick s1 left (deterministic tie-break).
            // Exception: when both segments share a coincident anchor at either end — converging on
            // essentially the same target point (a symmetric fan-in, e.g. two approaches converging on
            // one shared anchor) or diverging from essentially the same source point (a symmetric
            // fan-out, e.g. one shared anchor branching to two approaches) — forcing a strict left/right
