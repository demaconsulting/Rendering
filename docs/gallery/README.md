...
container's nested children. The external label reads outward (away from the container) and the internal label reads
inward, and the anchor consolidates external and internal fan-out onto a single boundary point.
...
from the sibling and both internal delegation edges (internal fan-out to two nested children) reach that one anchor.
...
sibling approach edges) consolidated onto the one shared anchor, which then delegates inward to the single nested child.
